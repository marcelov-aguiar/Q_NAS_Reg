...
class MultiHeadNetworkGraphNew(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            Tensor: Output tensor of shape (batch_size, num_windows, num_classes)
        """
        features = []
        ...
        if self.fc is None:
            total_feature_size = x_concat.shape[2]
            self.fc = nn.Linear(total_feature_size, self.num_classes)

        # Aplica camada totalmente conectada a cada timestep
        output = self.fc(x_concat)  # (batch, time, num_classes)
